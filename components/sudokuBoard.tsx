...
function SudokuBoard() {
    ...
    const renderItem = useCallback(
        ({
            ...
            item: { value: string; rowIndex: number; columnIndex: number };
        }) => (
            <Pressable
                onPressIn={() => {
                    setPlayerPos({
                ...
                style={[
                    styles.cell,
                    (playerPos.rowIndex === item.rowIndex
                        || (playerPos.columnIndex === item.columnIndex
                            && !(
                                playerPos.columnIndex === item.columnIndex
                                && playerPos.rowIndex === item.rowIndex
                            )))
                            && styles.rowColumnBackground,
                    ((playerPos.columnIndex === item.columnIndex
                        && playerPos.rowIndex === item.rowIndex)
                    || (!item.value.includes('-')
                        && item.value.includes(currentNumber)))
                    && styles.currentNumber,
                    (item.columnIndex + 1) % 3 === 0
                    && item.columnIndex !== 8
                    && styles.rightBorder,
                    (item.rowIndex + 1) % 3 === 0
                    && item.rowIndex !== 8
